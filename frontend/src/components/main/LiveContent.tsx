...
interface data {
  ...
  roomId: number;
  roomTitle: string;
  crowdList: { email: string; name: string; profilePic: string }[];
}

export default function LiveContent({ liveData }: { liveData: data }) {
  return (
    // 배너 이미지
    <Card
      className={`border-solid border-4 rounded-3xl bg-[url('${liveData.playerBackgroundPic}')] bg-cover h-[15rem] w-[23rem] grid grid-rows-5 lg:hover:scale-105 transition-transform ease-in-out duration-500`}
    >
      <CardHeader className="p-1 grid grid-cols-2">
        <Live />
        <div className="flex justify-end">
          {/* 채팅방에 들어온 친구 수 만큼 친구 프사 표시 */}
          {liveData.crowdList.map((crowd, id) => {
            return (
              <div
                key={id}
